...
const postregistroController = async(req,res)=>{
    ...
    /* opcion 2 */ fotofile.mv(`${path.join(__dirname,`../public/img/imagen_${id}.jpg`)}`, (err) => {  if (err) {console.log(err);
        res.redirect('/registro.html')}}); const fotoRuta = `img/imagen_${id}.jpg`
    ...
    await registro(nombre,email,password,parseInt(anos_experiencia), especialidad,estado, fotoRuta);
    res.redirect('/')
}
